...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: fileData, error: dlErr } = await serviceClient.storage.from("meal-images").download(storagePath);
    if (dlErr || !fileData) {
      return new Response(JSON.stringify({
        success: false,
    ...
    if (!bytes || bytes.byteLength === 0) {
      return new Response(JSON.stringify({
        success: false,
        error: "Image is empty (0 bytes) or unreadable"
      }), {
        headers: {
          ...corsHeaders,
          "Content-Type": "application/json"
...
`.trim();
    const userText = fixPrompt ? `User correction: "${fixPrompt}". Re-analyze accordingly and return ONLY the JSON object.` : "Analyze this food photo and return ONLY the JSON object described above.";
    const body = {
      model: "gpt-4o",
      temperature: 0.1,
      response_format: {
